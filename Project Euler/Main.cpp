...
//Includes for Euler Problems
#include "BigInt.h"
#include "Data.h"

//Math Functions
template <typename T>
inline bool isMultiple(T num, T mult);
inline bool isPalindromic(long n);
inline bool isPrime(long n);
inline int numOfDivisors(long n);
inline long sumOfDivisors(long n);
void quickSort(char *a, int bufLen, int lo, int hi);
int partition(char *a, int bufLen, int lo, int hi);
void swap(char *a, int bufLen, int i, int j);
long factorial(long n);
inline bool isPentagonal(long n);
...
void memTest()
{
	...
	puts (str);
	// memcpy is a too safe
	// it is really just memmove
...
{
	long a=1;
	for(int i=2; numOfDivisors(a) <= 500; ++i)
		a += i;
...
void problem21()
{
	...
	for(int i=2; i<10000; ++i){
		long m = sumOfDivisors(i);
		if(m>i){
			if(i == sumOfDivisors(m)){
				if(m < 10000)
					a += i+m;
				else
					a += i;
...
void problem22()
{
	...
	quicksort(&(names[0][0]), MAX_NAME_LEN, 0, TOTAL_NAMES-1);
	int nameValue;
...
inline bool isPentagonal(long long n)
...
long problem60r(long *primes, long *mask, int level, int prev)
{
	...
	for(int i=3; i<prev; i+=2){
		...
		for(int j=0; j<level; ++j){
			if(!isPrime(mask[j]*i+primes[j]))
				goto nextI;
			if(!isPrime(mask[level]*primes[j]+i))
				goto nextI;
		}
		primes[level] = i;
